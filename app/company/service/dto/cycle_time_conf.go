...
type CycleTimeConfUpdateReq struct {
	...
	Type      int    `json:"type" comment:"类型,每天,每周"`
	StartWeek int    `json:"startWeek" comment:"类型为周,每周开始天"`
	EndWeek   int    `json:"endWeek" comment:"类型为周,每周结束天"`
	StartTime string `json:"startTime" comment:"开始下单时间"`
	EndTime   string `json:"endTime" comment:"结束时间"`
	GiveDay   int    `json:"giveDay" comment:"跨天值为0是当天,大于0就是当天+天数"`
	GiveTime  string `json:"giveTime" comment:"配送时间,例如：15点至19点"`
	common.ControlBy
}
