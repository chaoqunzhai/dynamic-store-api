...
// @Security Bearer
func (e ShopCreditLog) GetPage(c *gin.Context) {
    req := dto.ShopCreditLogGetPageReq{}
    s := service.ShopCreditLog{}
    err := e.MakeContext(c).
        MakeOrm().
        Bind(&req).
	...
	list := make([]models.ShopCreditLog, 0)
	var count int64
	...
	var shopObjectList []models.Shop
	var shopObject models.Shop
	e.Orm.Model(&shopObject).Scopes(actions.PermissionSysUser(shopObject.TableName(),userDto)).Select("name,id").Where("id in ?",shopList).Find(&shopObjectList)
	shopRowMap:=make(map[int]string,0)
	for _,row:=range shopObjectList{
...
// @Security Bearer
func (e ShopCreditLog) Get(c *gin.Context) {
	//ShopBalanceLogGetReq
	req := dto.ShopCreditLogGetReq{}
	s := service.ShopCreditLog{}
    err := e.MakeContext(c).
		MakeOrm().
		Bind(&req).
	...
	var object models.ShopCreditLog
...
// @Security Bearer
func (e ShopCreditLog) Insert(c *gin.Context) {
	//ShopBalanceLogInsertReq
    req := dto.ShopCreditLogInsertReq{}
    s := service.ShopCreditLog{}
    err := e.MakeContext(c).
        MakeOrm().
        Bind(&req).
...
// @Security Bearer
func (e ShopCreditLog) Update(c *gin.Context) {
	//ShopBalanceLogUpdateReq
    req := dto.ShopCreditLogUpdateReq{}
    s := service.ShopCreditLog{}
    err := e.MakeContext(c).
        MakeOrm().
        Bind(&req).
...
// @Security Bearer
func (e ShopCreditLog) Delete(c *gin.Context) {
    s := service.ShopCreditLog{}
    req := dto.ShopCreditLogDeleteReq{}
    err := e.MakeContext(c).
        MakeOrm().
        Bind(&req).
        MakeService(&s.Service).
