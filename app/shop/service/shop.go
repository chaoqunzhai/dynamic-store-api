...
func (e *Shop) Insert(userDto *sys.SysUser, c *dto.ShopInsertReq) error {
	...
	if createErr==nil{
		...
		if len(strNumbersIds) > 0 {
			updateMap["china_id"] = strings.Join(strNumbersIds,",") //存ID
		}
		e.Orm.Model(&models.Shop{}).Where("id = ?",data.Id).Updates(updateMap)
		if c.ApproveId > 0 {
			//更新审批状态
