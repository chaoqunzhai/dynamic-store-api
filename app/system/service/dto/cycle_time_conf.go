...
type CycleTimeConfOrder struct {
	...
	GiveStartTime string `form:"giveStartTimeOrder"  search:"type:order;column:give_start_time;table:cycle_time_conf"`
	GiveEndTime   string `form:"giveEndTimeOrder"  search:"type:order;column:give_end_time;table:cycle_time_conf"`
}
...
type CycleTimeConfInsertReq struct {
	...
	Type          string    `json:"type" comment:"类型,每天,每周"`
	StartWeek     string    `json:"startWeek" comment:"类型为周,每周开始天"`
	EndWeek       string    `json:"endWeek" comment:"类型为周,每周结束天"`
	StartTime     time.Time `json:"startTime" comment:"开始下单时间"`
	EndTime       time.Time `json:"endTime" comment:"结束时间"`
	GiveDay       string    `json:"giveDay" comment:"跨天值为0是当天,大于0就是当天+天数"`
	GiveStartTime time.Time `json:"giveStartTime" comment:"配送开始时间"`
	GiveEndTime   time.Time `json:"giveEndTime" comment:"配送结束时间"`
	common.ControlBy
}
...
func (s *CycleTimeConfInsertReq) Generate(model *models.CycleTimeConf) {
	...
	model.GiveStartTime = s.GiveStartTime
	model.GiveEndTime = s.GiveEndTime
}
