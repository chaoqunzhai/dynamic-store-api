 ...
 */
func ResolveSplitSearchQuery(table,driver string, q interface{}, condition Condition) {
	...
	for i := 0; i < qType.NumField(); i++ {
		...
		if !ok {
			//递归调用
			ResolveSplitSearchQuery(table,driver, qValue.Field(i).Interface(), condition)
			continue
		}
		switch tag {
		case "-":
		...
		if driver == Postgres {
			pgSql(table,driver, t, condition, qValue, i)
		} else {
			otherSql(table,driver, t, condition, qValue, i)
		}
	}
}

func pgSql(table,driver string, t *resolveSearchTag, condition Condition, qValue reflect.Value, i int) {
	switch t.Type {
	case "left":
		//左关联
		...
		ResolveSplitSearchQuery(table,driver, qValue.Field(i).Interface(), join)
	case "exact", "iexact":
...
func otherSql(table,driver string, t *resolveSearchTag, condition Condition, qValue reflect.Value, i int) {
	switch t.Type {
	case "left":
		//左关联
		join := condition.SetJoinOn(t.Type, fmt.Sprintf(
